...
jq --arg version "$new_version" '.version = $version' composer.json > composer.tmp && mv composer.tmp composer.json
...
echo "Version updated to $new_version and merged into main and develop branches."
